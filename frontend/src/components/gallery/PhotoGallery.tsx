  ...
  return (
    <>
      <div className="relative grid h-[52vh] max-h-[520px] min-h-[340px] grid-cols-1 gap-2 overflow-hidden rounded-2xl md:grid-cols-2">
        ...
        <button
          onClick={() => setOpen(true)}
          className="absolute bottom-4 right-4 flex items-center gap-2 rounded-lg border border-ink bg-bg px-3 py-1.5 text-sm font-medium shadow-sm hover:scale-[1.02]"
        >
      ...
      {open && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-black/95" onClick={() => setOpen(false)}>
          <button
            ...
            className="fixed left-4 top-4 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-bg/90 text-2xl leading-none hover:bg-bg"
          >
            ×
          </button>
